...
- ### 3.1 Catastrophic Cancellation
    ...
    - ### 3.2 Floating Point Comparison
        부동소수점에서 '거의 같음'을 어떻게 판단하고, 판단하는 기준을 무엇인가? 
        : 모든상황에 같은 eplsion값을 사용하면 문제가 될 수 있습니다. 
        1. 단순히 부동소수점을 비교하면 수학적으로 문제되는 결과가 발생하게됩니다.
            - 예시(cout << (0.11f + 0.11f)<0.22f);) 와 같이 비교한 true가 나옵니다.
        2. 절대 오차(epsilon)을 사용하여 비교하면, 작은수와 비교하는 경우 사용자에 의해 고정된 epsilon값이 너무 크거나, 큰 숫자와 비교할 때는 최소 반올림 오차보다 작아서 항상 거짓을 반환 할 위험이 있습니다.
        3. 2번에 대한 해결책으로, 상대오차를 이용하여 해결할 수 있습니다.
            - |a-b|/b < eplision
        4. 나눗셈 기반 상대오차 비교방식은, 0으로 나누는 경우(b가 0인경우) 등 몇가지 문제가 발생될 여지가있습니다.
            - a와 b가 매우작은경우, 오차가 커질 가능성
            - 또한 교환법칙이 성립하지 않습니다. 
        5. 최종적으로 개선된 방법
            - 두 값의 차이를 두 값 중 큰 값으로 나눈 상대 오차를 계산하며, 이 상대오차가 사용자가 정한 eplision보다 작을때, 두 수를 거의 같다고 표현합니다.
            - |a-b| / max(|a|,|b|) < e 와같은방식으로 표현됩니다.
            - inf 혹은 NaN인 경우를 제외하는 로직또한 추가합니다.
            - 1.0000001,과 1.0000002를 비교할때, 두 수의 차이는 1e-6의 경우, eplision인 1e-6보다 작으므로 '거의같음' 입니다.  
